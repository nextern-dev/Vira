...
export async function createSession(userId: string): Promise<void> {
  ...
  const [appearance] = await db
    .select({
      mode: users.appearanceMode,
      theme: users.colorTheme,
    })
    .from(users)
  ...
  store.set(SESSION_COOKIE, token, {
    httpOnly: true,
    // Arena renders the preview cross-site in an iframe. The API's strict
    // Origin guard remains the CSRF boundary for all state-changing requests.
...
/** Reads and validates the session cookie. Returns null when unauthenticated. */
  ...
  try {
    const rows = await db
      .select({
        ...
        appearanceMode: users.appearanceMode,
...
/** Signs every *other* device out — used after a credential change. */
...
/** Clears non-Vira cookies that could quietly re-authorise a user. */
...
export async function destroyCurrentSession(): Promise<void> {
  ...
  // Appearance is intentionally retained: it is non-sensitive UI state and
  // keeps public/landing pages visually consistent after sign-out.
}

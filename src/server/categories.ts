...
export async function assertCategoryOwned(
  ...
  if (expectedKind && category.kind !== expectedKind) {
    throw conflict(
...
export async function createCategory(
  userId: string,
  input: { name: string; kind: "income" | "expense"; color: string; icon: string },
): Promise<Category> {
  try {
    const [row] = await db
      .insert(categories)
    ...
    if (isUniqueViolation(error)) {
      throw conflict(`You already have a ${input.kind} category called "${input.name}".`);
    }
    throw error;
  }
}
...
export async function deleteCategory(
  ...
  if (reassignToId) {
    if (reassignToId === categoryId) {
      throw conflict("Pick a different category to move transactions into.");
    }

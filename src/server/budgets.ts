...
/**
 * Budget spend is computed in a single aggregate query directly against the
 * ledger — never from cached or client supplied numbers. Only *expenses* owned
 * by the user, inside the budget window and matching the budget's category
 * qualify.
 */
export async function listBudgets(userId: string): Promise<BudgetWithProgress[]> {
  ...
  return rows.map((row) => {
    ...
    const spentCents = row.isActive ? spend.spent : spend.spent;
    const percentUsed =
      row.limitCents > 0 ? (spentCents / row.limitCents) * 100 : 0;
    return {
      ...row,
      windowFrom: from,
      windowTo: to,
